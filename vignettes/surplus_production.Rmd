...
```{r, echo=TRUE, message=TRUE, fig.width=7, fig.height=7}
Phat_t = (out$rep$g_ti[,1] - out$rep$m_ti[,1]) * out$rep$B_ti[,1]
Bhat_t = out$rep$B_ti[,1]

x = seq(0, K, length=1000)
y = r * x * (1 - x/K)
plot( x=x, y=y, type="l", lwd=2, xlim=c(0,2*K), ylim=c(0,2*max(y)) )
points( x=Bhat_t*exp(out$internal$parhat$logq_i), y=Phat_t*exp(out$internal$parhat$logq_i) )
```
