...
prod_fun = function( biomass, Vprime, logPB, taxon, ecofit ){
...
phi1 = optimize( dBdt_approx, lower=0.01, upper=0.99, maximum=TRUE,
                x = 1+exp(out$internal$parhat[['Vprime_ij']]), 
...
pars = out0$tmb_inputs$p
map = out0$tmb_inputs$map
#map$Vprime_ij = factor( rep(1,n_taxa) %o% ifelse(type=="auto",NA,1) )
...
yhat_bivar = sapply( x, FUN=prod_fun, ecofit=out_bivar, taxon="target",
               Vprime = out_bivar$internal$parhat$Vprime_ij, 
