...
taxa = c("Producer", "Consumer1", "Predator1", "Consumer2", "Predator2")
n_taxa = length(taxa)
...
DC_ij = matrix( c(
  ...
  0, 0, 0, 0,   0.5,
  0, 0, 0, 0,   0
), byrow=TRUE, ncol=length(taxa) )
# Ratio of production to biomass:  Reciprocal of mean age according to Polovina-1984 ~= M
PB_i = c( 90, 5, 0.2, 3, 0.1 )         
# Ratio of consumption to biomass
QB_i = c( 150, 10, 3, 15, 4 )
# Functional response parameters
V_ij = matrix( 1.5, nrow=n_taxa, ncol=n_taxa )
# Equilibrium biomass
B0_i = c(1, 1, 1, 1, 1)
...
deltaB_ti = rarray(dims=c(n_years,n_taxa),sd=1) * outer(rep(1,n_years),sigmaB_i)
...
# Solving for EE but passing vector of NAs as placeholder
EE_i = rep(NA, n_taxa)

# Label EwE inputs for each taxon as expected (so users can easily change taxa)
names(PB_i) = names(QB_i) = names(B0_i) = names(EE_i) = taxa
