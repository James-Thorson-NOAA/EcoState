...
pars = list( logB_i = log(B0_i),
             ...
             epsilon_i = rep(0,n_taxa),
             logF_i = rep(-Inf,n_taxa),
...
#type = factor(c("auto","detritus","hetero","hetero","hetero","hetero"), levels=c("auto","hetero","detritus"))

# Label EwE inputs for each taxon as expected (so users can easily change taxa)
names(PB_i) = names(QB_i) = names(fittedB_i) = names(fittedEE_i) = names(type) = taxa
